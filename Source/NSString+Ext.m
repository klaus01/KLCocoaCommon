...
+ (NSString *)UUID
{
    CFUUIDRef uuidDef = CFUUIDCreate(NULL);
    CFStringRef uuidStringRef = CFUUIDCreateString(NULL, uuidDef);
    CFRelease(uuidDef);
...
{
    CFUUIDRef uuidDef = CFUUIDCreate(NULL);
    CFUUIDBytes uuidBytes = CFUUIDGetUUIDBytes(uuidDef);
    CFRelease(uuidDef);
...
- (NSString *)trim
{
    ...
    [mutStr setString:self];
    CFStringTrimWhitespace((CFMutableStringRef)mutStr);
    return [mutStr autorelease];
}

#pragma mark - firstPin
...
/// 获取拼音首字母
char pinyinFirstLetter(unsigned short hanzi ,int isUpper)
{
	int index = hanzi - HANZI_START;
	if (index >= 0 && index <= HANZI_COUNT)
        return firstLetterArray[index] - (isUpper == 0 ? 0 : 32);//大写输出
	else
		return hanzi;
}
...
- (NSString *)firstPin
{
    ...
    char *tmp = (char *)malloc(self.length + 1);
    if (tmp)
    {
        int i = 0;
        for (; i < self.length; i++)
            tmp[i] = pinyinFirstLetter([self characterAtIndex:i] ,1);
        tmp[i] = '\0';
...
- (NSString *)allPin
{
    ...
    for (int j = 0; j < self.length; j++)
    {
        ...
        NSData *data = [[self substringWithRange:rang] dataUsingEncoding:enc];
        ...
        if (zi.length < 4)
        {
            [ret appendString:[self substringWithRange:rang]];
            continue;
        }
        ...
        if((0x10 <= bh) && (bh <= 0x57))//是gb2312汉字
        {
            ...
            {
                NSRange  tRang = [pinyinTable[j].hanzi rangeOfString:[self substringWithRange:rang]];
                
                if (tRang.location != NSNotFound)
                {
                    [ret appendString:[pinyinTable[j].pinyin lowercaseString]];
                    isFind = true;
                    break;
                }
